				...
				<?php
					...
					if ( $resume_access_level == 'subscribe' ):
						...
						<p class="meta"><?php 
							...
							if ($desired_salary = get_post_meta($post->ID, '_desired_salary', true)) :
								echo sprintf( __('<br/>Desired salary: <strong>%s</strong>. ', APP_TD), jr_get_currency($desired_salary) );
							endif;
					...
					<div class="section_content">
						...
						<?php if (get_the_author_meta('ID')==get_current_user_id()) : ?>
							<p class="button edit_resume"><a href="<?php echo add_query_arg('edit', $post->ID, get_permalink(get_option('jr_job_seeker_resume_page_id'))); ?>"><?php _e('Edit Resume&nbsp;&rarr;',APP_TD); ?></a></p>
						<?php endif; ?>
