...
class JR_Widget_pack_pricing extends WP_Widget {
    ...
    function widget( $args, $instance ) {
    	...
    	foreach ($packs as $pack) :
            ...
            if (!$pack->job_count) $pack->job_count = __('Unlimited', APP_TD);

            if ($pack->pack_duration) $pack->pack_duration = __(' usable within ', APP_TD).$pack->pack_duration.__(' days', APP_TD);

            if ($pack->job_duration) $pack->job_duration = __(' lasting ', APP_TD).$pack->job_duration.__(' days' ,APP_TD);

			if ($pack->pack_cost) :
				$pack->pack_cost = jr_get_currency($pack->pack_cost).''; 
				$cost = 1; 
            else:
				$pack->pack_cost = __('Free',APP_TD);
			endif;	
			
            echo '<li><span class="cost">'.$pack->pack_cost.'</span><p><strong>'.$pack->pack_name.'</strong><br />'.$pack->job_count.' '.__('Jobs', APP_TD).''.$pack->job_duration.$pack->pack_duration.'.</p>'.
			($buy == 'yes' && $cost > 0?'<p class="button buy-pack"><a href="'.add_query_arg(array('tab' => 'packs', 'pack_id' => $pack->id), get_permalink(get_option('jr_dashboard_page_id'))).'">Buy</a></p>':'').'</li>';
...
?>
		...
		<p>
			<input type="checkbox" id="<?php echo $this->get_field_id( 'buy' ); ?>" name="<?php echo $this->get_field_name( 'buy' ); ?>" value="yes" <?php echo ($buy=='yes'?'checked':''); ?>> <?php _e( 'Enable <i>Buy Now</i> button',APP_TD); ?>
		</p>
		<?php endif; 
	}
}
...
class JR_Widget_Job_Categories extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( $d ) {
			...
			if ($terms) foreach ($terms as $term) $output .= "<option value='".$term->slug."'>".$term->name."</option>";
			$output .= "</select>";
			echo $output;
?>
   ...
   ?>
		...
		<p>
			...
			<select class="widefat" id="<?php echo $this->get_field_id('type'); ?>" name="<?php echo $this->get_field_name('type'); ?>" >
				<option value="<?php _e('username') ?>" <?php if ('username' == $instance['type']) echo 'selected="selected"'; ?>><?php _e('Show Username Tweets', APP_TD) ?></option>
				<option value="<?php _e('keywords') ?>" <?php if ('keywords' == $instance['type']) echo 'selected="selected"'; ?>><?php _e('Show Keyword Tweets', APP_TD) ?></option>
			</select>
		</p>
