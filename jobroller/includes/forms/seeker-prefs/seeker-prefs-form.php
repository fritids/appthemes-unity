...
function jr_job_seeker_prefs_form() {
	...
	<form action="<?php echo get_permalink( $post->ID ); ?>" method="post" id="submit_form" class="submit_form main_form">
		...
		<fieldset>
			...
			<!--<p><label for="where_you_can_work"><?php _e('Where are you authorised to work?', APP_TD); ?></label> 
			<input type="text" class="tags text" name="where_you_can_work" id="where_you_can_work" placeholder="<?php _e('e.g. United States, United Kingdom', APP_TD); ?>" value="<?php echo $where_you_can_work; ?>" /></p>-->
