...
add_action('jr_after_insert_job','jr_job_alerts_set_new');
...
function jr_job_alerts_alert_type( $post_id ) {	
	...
	if ( !empty($posted['jr_geo_short_address']) ) 
		$location .= '|location=' . trim($posted['jr_geo_short_address']) . '.*';
	
	$alert .= '(' . $location . ')';
			
	if ( !empty($posted['job_term_type']) ) 
		$job_type .= '|job_type=' . get_term_by('slug',$posted['job_term_type'],APP_TAX_TYPE)->term_id;
	
	$alert .= '.*(' . $job_type . ')';
	
	if ( !empty($posted['job_term_cat']) ) 
		$job_cat .= '|job_cat=' . trim($posted['job_term_cat']);
 ...
 *
 */
function jr_job_alerts_set_new( $post_id ) {
	global $wpdb, $app_abbr;
	
	if ( get_option($app_abbr.'_job_alerts') == 'no' ) return;
...
function jr_job_alerts_read_template( $template = '' ) {
	...
	$template_path = apply_filters('jr_job_alerts_template_path', TEMPLATEPATH)	;
...
function jr_job_alerts_get_templates( $template_pattern = '*tmpl-*.html' ) {
	...
	foreach (glob($template_pattern) as $filename) {
		$tmpl = array ( basename( $filename ) => __('External HTML Template ('.basename( $filename ).')', APP_TD) ); 		
