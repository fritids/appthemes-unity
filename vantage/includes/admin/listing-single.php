...
class VA_Listing_Pricing_Meta extends APP_Meta_Box{
	...
	public function before_display( $form_data, $post ){
		...
		if( !empty ( $form_data['featured-home_start_date'] ) )
			$form_data['featured-home_start_date'] = mysql2date( $date_format, $form_data['featured-home_start_date']); 

		if( !empty ( $form_data['featured-cat_start_date'] ) )
			$form_data['featured-cat_start_date'] = mysql2date( $date_format, $form_data['featured-cat_start_date']);
