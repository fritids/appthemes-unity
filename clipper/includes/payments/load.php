...
function _appthemes_load_payments() {
	...
	require dirname( __FILE__ ) . '/gateways/bank-transfer/bt-emails.php';
...
function appthemes_payments_get_args(){
	...
	$defaults = array(
		...
		'options' => false,
		'currency_default' => $format['currency_default'],
		'currency_format' => $format['currency_format'],
		'hide_decimals' => $format['hide_decimals']
	);
...
function _appthemes_load_price_format() {
	...
	require dirname( __FILE__ ) . '/format/currencies.php';
	require dirname( __FILE__ ) . '/format/number-format.php';
}

function appthemes_price_format_get_args( $force = false ){
	...
	if( !in_array( $args['currency_format'], array( 'symbol', 'code') ) ){
		$args['currency_format'] = 'symbol';
	}
